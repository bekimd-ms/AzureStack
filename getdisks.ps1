Get-Azsdisk -Status All |  select `
      ...
      @{Name="DiskResourceGroup";Expression={ $m = $_.UserResourceId -match "/subscriptions/(.*)/resourceGroups/(.*)/providers/Microsoft.Compute/Disks/(.*)"; $matches[3] }}, `
      @{Name="DiskSubscription";Expression={ $m = $_.UserResourceId -match "/subscriptions/(.*)/resourceGroups/(.*)/providers/Microsoft.Compute/Disks/(.*)"; $matches[3] }}, `
      ProvisionSizeGB, `
      ActualSizeGB, `
      DiskSku `
| sort Volume
